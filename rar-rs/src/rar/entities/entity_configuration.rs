...
impl EntityConfigurationState {
	...
	pub fn directions_iter( &self ) -> std::collections::hash_map::Iter< String, EntityConfigurationStateDirection > {
...
impl EntityConfiguration {
	...
	pub fn states_iter( &self ) -> std::collections::hash_map::Iter< String, EntityConfigurationState > {
		self.states.iter()
	}
}
...
impl EntityConfigurationManager {
	...
	pub fn load_yaml(&mut self, system: &mut System, filename: &str) -> anyhow::Result<()> {
		...
		for (k, v) in ecye.states {
			let mut s = EntityConfigurationState::new(&k, v.first_frame, v.last_frame, &v.size, &v.offset);
